#include "dualsense/DualSenseDescriptor.h"

#include <array>

namespace asb::dualsense {
namespace {

// Transcribed from the ApexSenseBridge VIIPER fork
// (third_party/viiper-patches/viiper-v0.7.0-asb.patch,
// device/dualsense/asb_descriptor.go).
constexpr std::array<std::uint8_t, kUsbReportDescriptorSize> kUsbReportDescriptor{{
    ,}};

static_assert(kUsbReportDescriptor.size() == kUsbReportDescriptorSize);

} // namespace

std::span<const std::uint8_t> usbReportDescriptor() noexcept {
    return {kUsbReportDescriptor.data(), kUsbReportDescriptor.size()};
}

} // namespace asb::dualsense
