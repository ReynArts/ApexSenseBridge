...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $projectRoot "dist\experimental\viiper-v0.7.0-asb5.exe"
} else {
...
Copy-Item -LiteralPath (Join-Path $sourceDirectory "LICENSE.txt") `
    -Destination (Join-Path $outputDirectory "VIIPER-v0.7.0-asb5-LICENSE.txt") -Force
...
) | Set-Content -LiteralPath (Join-Path $outputDirectory "VIIPER-v0.7.0-asb5-SOURCE.txt") `
    -Encoding UTF8
